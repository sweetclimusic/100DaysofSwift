...
//Combine observer for the currently selected Pizza to order.
class SelectedItem: ObservableObject{
    static var current = SelectedItem()
    private init(){}
    @Published var selected:MenuItem!
}
...
struct MenuModel{
   static var menu:[MenuItem] = [
        ...
        MenuItem(id: 4,name: "The Big Island", description: "A meaty calzone exploding like a volcano. Beef and pork combined with vegetables, pineapple, and a special \"lava sauce\" leaking out the top crater. Definitely share this one.", price: 17.00, rating: 3),
